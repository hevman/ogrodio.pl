              ...
              {products.map((product) => (
                <article className="flex h-full min-h-[470px] flex-col overflow-hidden rounded-lg border border-emerald-900/10 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md" key={product.variantId}>
                  <Link href={`/produkt/${product.slug}`}>
                    <span className={`relative block bg-slate-50 ${product.category === "ebooki" ? "mx-auto aspect-[210/297] w-full max-w-[240px]" : ""}`}>
                      <img className={`${product.category === "ebooki" ? "h-full" : "h-52 2xl:h-56"} w-full object-contain p-3 transition duration-200 hover:scale-[1.02]`} src={productImage(product)} alt="" />
